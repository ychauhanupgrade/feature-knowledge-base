...
**E2E automation is under construction in qa-automation#38621 (open).** At init there was none: the only prior qa-automation PR (#37369, merged) extended `AllureId 21161` to assert `merchantId` on the `projectStatusChanged` Kafka event — a data-plumbing prerequisite for R2, not a fraud rule. #38621 adds the missing scaffolding (`fraud_rule_hit` queries and accessors, a `HomeImprovementFraudRule` enum, a `waitForFraudRuleHits` poll helper, a `HI_FRAUD_TRIGGERS_AT_PAYMENT_REQUEST` feature constant, and a `createNewMerchantEmployeeUnderNewMerchant` overload taking merchant categories) plus one E2E per shipped rule. **R2 is validated — `AllureId 87944` passed end to end on preprod (Jenkins 6858), proving the full Flink→HIMS chain works in a real environment.** R4 (87945) and R5 (87920) are written but not yet green. Service-level UT/IT inside `hi-fraud-flink-job` remains genuinely strong (per-rule evaluator, filter, state-retention and pipeline-harness tests plus `RapidProjectCompletionIT` / `SameDeviceIT`), but it runs against a Flink test harness with synthetic events — which is precisely why the R2 pass matters.

Two rules are deliberately **not** covered by E2E and should not be treated as gaps to close now: R1 Same Device is half-landed (HIMS#6180 merged, but evaluator `hi-fraud-flink-job#58` is still open and the job is deployed to no environment, so a test would fail on every run), and R3 Employee Email is being **deleted** by six open PRs. Per Charles Chartrand's review of this page (2026-09-17), the target is at most one E2E per rule, so the threshold-boundary and `HVAC_PLUS` discriminator cases stay at the unit layer in `RapidCompletionEvaluatorTest` rather than becoming additional E2Es.
...
| HI-7625 | [BE][hi-fraud-flink-job] Rule 1 — Same Device — Flink job implementation | Ready for CodeReview | Done | Done | GAP | hi-fraud-flink-job#58 (**still OPEN**), #65 (merged); home-improvement-merchant-srvc#6180 (**MERGED since last refresh**); avro-home-improvement-lib#1034. Half-landed: the HIMS data prerequisite is in, the evaluator is not. |
| HI-7960 | [BE][hi-fraud-flink-job] Rules 4 & 5 — Rapid Project Completion — Flink job implementation | Closed | Done | Done | IN DEV | hi-fraud-flink-job#8, #10; k8s-template#284065, #282695, #282630, #282570; terraform_modules#7406 (declined), stage_terraform#4343 (draft), k8s-template-charts#5063 (declined); **qa-automation#38621 (open)** |
...
| R4 Rapid completion, non-HVAC ≥ $50,000 within 2 days | Done | Done | IN DEV | HI-7960. `RapidCompletionEvaluatorTest`, `MasterLineProjectJoinTest`, `RapidCompletionFiltersTest`, `SnapshotTypesTest`, `StateRetentionTest`, `RapidProjectCompletionIT`. Rule id `RAPID_COMPLETION_NON_HVAC`. Deployed ondemand/stage/preprod/prod. E2E `AllureId 87945` (qa-automation#38621) written, **not yet run** — needs the new category overload to seed a PLUMBING merchant. |
| R5 Rapid completion, HVAC ≥ $30,000 within 2 days | Done | Done | IN DEV | HI-7960. Rule id `RAPID_COMPLETION_HVAC`. **`HVAC_CATEGORIES = Set.of("HVAC")` — `HVAC_PLUS` is deliberately excluded and falls to the R4 $50K threshold** (`hi-fraud-flink-job#56`), contradicting HI-7455 AC4 and HI-7960's own rule text. E2E `AllureId 87920` (qa-automation#38621) written; **not yet green** — first two runs failed on environment and test-setup causes, not on the rule. |
...
| `previousStatus` on payment-request event | Done | N/A | GAP | HI-7625. `PaymentRequestServiceTest` in HIMS#6180, **merged since last refresh**; `hi-fraud-flink-job#65` classifies transitions on it. |
...
- [MEDIUM, was HIGH] **E2E coverage of rule firing is now partly in place.** R2 is validated end to end on preprod (`AllureId 87944`, Jenkins 6858). R4 (87945) and R5 (87920) are written in qa-automation#38621 but not yet green — R5's two failures were an environment stall and a test-setup defect, not the rule; R4 has not executed. Remaining work is getting those two green, not writing them.
- [HIGH] **HI master-line opening stalls on stage**, blocking any HI E2E there. Jenkins 6851: the loan reached APPROVED, `ArixHomeImprovementStatusSyncJob` and `HomeImprovementCreditLineReadyToOpenJob` all COMPLETED, then `loanreview.account_candidate` never received a row across 10 minutes of polling, so `approveAndOpenHomeImprovementMasterLine` timed out. Preprod does not reproduce it. Shared framework/platform territory rather than this epic — worth raising with the HI team independently.
